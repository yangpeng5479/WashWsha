...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Silver";
...
- (void)loadWalletData {
    ...
    [DataService postWithURL:@"rest3/v1/Exchange/get_official_exchange_diamond" type:1 params:dic fileData:nil success:^(id data) {
        [hud hideAnimated:YES];
        
        if ([data[@"status"] integerValue] == 1) {
            NSLog(@"%@",data);
            _gold = [data[@"data"][@"wallet"][@"coin_total"] integerValue];
            ...
            for (GoldModel *model in _modelMarr) {
                ...
                [_getGoldMarr addObject:model.get_coin_total];
                [_payMarr addObject:model.pay_diamond_total];
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        imageview.image = [UIImage imageNamed:@"gold-card"];
        [cell addSubview:imageview];
        
        UILabel *goldLabel = [[UILabel alloc] init];
        goldLabel.font = [UIFont systemFontOfSize:24];
        goldLabel.textColor = [UIColor whiteColor];
        goldLabel.attributedText = [DataService createAttributedStringWithImageName:@"icon-bigsilver" bounds:CGRectMake(0, -2, 25, 25) str:[NSString stringWithFormat:@"%ld",_gold]];
        [goldLabel sizeToFit];
        ...
        iconImageView.image = [UIImage imageNamed:@"icon-silver"];
        ...
        coinLabel.text = [NSString stringWithFormat:@"%@ Silver",[_getGoldMarr objectAtIndex:indexPath.row]];
        ...
        [payBtn setTitle:[NSString stringWithFormat:@"%@ Gold",_payMarr[indexPath.row]] forState:UIControlStateNormal];
...
- (void)payAction:(UIButton *)sender {
    ...
    NSDictionary *dic = @{@"token":[AccountManager sharedAccountManager].token,@"official_exchange_item_id":model.official_exchange_diamond_id};
    [DataService postWithURL:@"rest3/v1/Exchange/exchange_diamond_to_coin" type:1 params:dic fileData:nil success:^(id data) {
        [hud hideAnimated:YES];
        if ([data[@"status"] integerValue] == 1) {
