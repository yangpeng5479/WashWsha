...
static CGFloat const kNameLabelFont = 17.0;//送礼者
...
@property (nonatomic, weak) UIImageView *toIconIV; //目标头像
...
- (void)setModel:(LiveGiftShowModel *)model{
    ...
    self.sendLabel.text = [NSString stringWithFormat:@"Give %@",model.user.toName];
    
    [self.iconIV sd_setImageWithURL:[NSURL URLWithString:model.user.iconUrl] placeholderImage:[UIImage imageNamed:@"icon-people"]];
    if ([model.user.toIconUrl isEqualToString:@""]) {
        self.toIconIV.hidden = YES;
    }else {
        self.toIconIV.hidden = NO;
        [self.toIconIV sd_setImageWithURL:[NSURL URLWithString:model.user.toIconUrl] placeholderImage:[UIImage imageNamed:@"icon-people"]];
    }
    self.giftIV.image = [UIImage imageNamed:model.giftModel.picUrl];
...
- (void)setupContentContraints{
    ...
    [self.toIconIV mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (UIImageView *)backIV{
    if (!_backIV) {
        ...
        _backIV.image = [UIImage imageNamed:@"gift-bg"];
    }
    return _backIV;
}
...
- (UIImageView *)iconIV{
    if (!_iconIV) {
        ...
        _iconIV.image = [UIImage imageNamed:@"icon-people"];
...
- (UIImageView *)toIconIV {
    if (!_toIconIV) {
        _toIconIV = [self creatIV];
        _toIconIV.image = [UIImage imageNamed:@"icon-people"];
        _toIconIV.layer.cornerRadius = 15;
